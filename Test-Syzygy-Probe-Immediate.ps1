...
if (-not (Test-Path $stockfishPath)) {
    Write-Error "Stockfish introuvable à $stockfishPath"
    exit 1
}
...
# Positions de test qui sont dans les tables Syzygy (3pc à 5pc+)
$positions = @(
    @{ 
        Name = "KPK (3pc basique - position du projet)"
        FEN = "8/8/8/8/8/8/4P3/4K2k w - - 0 1"
    },
    @{ 
        Name = "KRPvKR (5pc classique)"
        FEN = "8/8/8/8/8/8/4k3/4P3/4K2R w - - 0 1"
    }
)
