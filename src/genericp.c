...
static uint64_t inline MakeMove(uint64_t idx, int k, int sq)
{
...
// portable replacements for the GCC bts/adcl asm (same semantics: v += old bit value)
#define bit_set(x,y) ((x) |= (1ULL << (y)))

#define bit_set_test(x,y,v) { uint64_t bit_set_test_b = (1ULL << (y)); (v) += ((x) & bit_set_test_b) ? 1 : 0; (x) |= bit_set_test_b; }
